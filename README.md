...
<h1 align="center"> SWOYO </h1>

Реализация 3-х функций по тестовому заданию
<!---------------------------------------------------------------------------------->
...
1. [ Описание ](https://github.com/SkorEgor/SWOYO#-простые-числа-в-заданном-диапазоне-)
2. [ Простые числа в заданном диапазоне ](https://github.com/SkorEgor/SWOYO#-простые-числа-в-заданном-диапазоне--1)

<!---------------------------------------------------------------------------------->
<h2 align="left"> Простые числа в заданном диапазоне </h2>

### Структура проекта

1. Простые числа в заданном диапазоне.
   1. prime_numbers.py - содержит функцию prime_numbers(low, high)
   2. test_prime_numbers.py - файл теста prime_numbers
2. Статистика текста
   1. text_stat.py - содержит функцию text_stat(filename) и ряд других для вычисления статистики
   2. test_text_stat.py - файл теста text_stat
   3. data_for_tests.py - данные для тестов test_text_stat
3. Перевод числа, состоящего из Римских цифр, в целое число
   1. roman_numerals_to_int.py - содержит функцию roman_numerals_to_int(roman_numeral)
   2. test_roman_numerals_to_int.py - файл теста roman_numerals_to_int

## Зависимости
1. Простые числа в заданном диапазоне - **math и numpy**
2. Статистика текста  - **re**
3. Перевод числа, состоящего из Римских цифр, в целое число - **re**

### Тестирование

Для тестирования используется модуль **pytest**. Для запуска достаточно прописать
...
### Задание
* Разработать функцию prime_numbers(low, high)
* low и high – нижняя и верхняя границы диапазона
* Возвращает список с простых чисел по возрастанию
* Обрабатывать некорректное значение аргументов, возвращая пустой список

### Реализация
В ходе поиска существующих решений были выбранные функции с наименьшим временем и предложенные свои реализации на основе уже существующих алгоритмов
...
```
---
Для тестирования производительности написан следующий код. 

Постепенно увеличиваем границы, перебираем функции и для каждой попытки делаем несколько вычислений. Результат с выбранным диапазоном заносим строчкой в DataFrame.
...
По критерию времени лучше всех показал вариант 4, моей реализации с numpy. 
Он отлично подходит для работы на больших диапазонах (>500). 

Итог: в результате используем prime_numbers_numpy
